...
class ListAdapter : BaseAdapter<LostItem>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int)
            : BaseHolder<LostItem> = ListHolder(ListItemBinding.inflate(LayoutInflater.from(parent.context), parent, false))
    ...
    inner class ListHolder(private val binding: ListItemBinding) : BaseHolder<LostItem>(binding.root) {
        override fun bind(data: LostItem) {
            ...
            binding.data = data
            binding.root.setOnClickListener { Log.w("start DetailActivity : ${data.id}") }
        }
    }
}
